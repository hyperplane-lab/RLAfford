...
class PAPO2OA2O(PAPRaw) :
    ...
    def compute_point_cloud_state(self) :
        # root tensor: 0: franka, 1: object, 2: hook
        ...
        selected_pc = self.sample_points(self.obj_pc, self.objPCDownsampleNum, sample_method='random')
        ...
        obj_pc_shape = selected_pc.shape
        lfinger_shape = selected_lfinger_pc.shape
        rfinger_shape = selected_rfinger_pc.shape
        if self.moving_pc_mode:
            obj_point_clouds = quat_apply(obj_r.view(-1, 1, 4).repeat_interleave(obj_pc_shape[1], dim=1), selected_pc[:, :, :3].float()) + obj_p.view(-1, 1, 3)
        else:
            obj_point_clouds = selected_pc
