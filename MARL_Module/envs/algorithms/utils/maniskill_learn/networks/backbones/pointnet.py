...
from mani_skill_learn.networks.modules.activation import build_activation_layer
import ipdb
...
class PointNetWithInstanceInfoV0(PointBackbone):
	...
	def forward_raw(self, data):
		...
		pcd = {'xyz': data[:, :, :3], 'seg': data[:, :, 3:3+self.mask_dim]}
		state = data[:, 0, 3+self.mask_dim:]
...
from algorithms.utils.maniskill_learn.utils.meta import build_from_cfg
...
def getPointNet(cfg) :
	...
	nn_cfg=dict(
		...
		conv_cfg=dict(
			type='ConvMLP',
			norm_cfg=None,
			mlp_spec=[cfg["input_feature_dim"], 256, 256],
			bias='auto',
			inactivated_output=False,
			conv_init_cfg=dict(
				type='xavier_init',
...
def getNewPointNet(cfg) :
	...
	nn_cfg=dict(
		...
		conv_cfg=dict(
			type='ConvMLP',
			norm_cfg=None,
			mlp_spec=[cfg["input_feature_dim"], 192, 192],
			bias='auto',
			inactivated_output=False,
			conv_init_cfg=dict(
				type='xavier_init',
		...
		state_cfg=dict(
			type='LinearMLP',
			norm_cfg=None,
			mlp_spec=[cfg["state_dim"], 192, 192],
			bias='auto',
			inactivated_output=False,
			linear_init_cfg=dict(
				type='xavier_init',
		...
		mlp_cfg=dict(
			type='LinearMLP',
			norm_cfg=None,
			mlp_spec=[384 * stack_frame, 192, cfg["feat_dim"]],
			bias='auto',
			inactivated_output=False,
			linear_init_cfg=dict(
				type='xavier_init',
	...
	nn_cfg=dict(
		...
		pcd_pn_cfg=dict(
			...
			conv_cfg=dict(
				type='ConvMLP',
				norm_cfg=None,
				mlp_spec=[cfg["state_dim"] + cfg["pc_dim"], 192, 192],
				bias='auto',
