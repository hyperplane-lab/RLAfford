from curses import KEY_SUSPEND, is_term_resized
...
class Runner:
    ...
    def __init__(self,
        ...
        config["learn"]["n_rollout_threads"] = vec_env.task.num_envs    # override the config
        if is_testing :
            config["learn"]["n_eval_rollout_threads"] = vec_env.task.num_envs    # override the config
        else :
            config["learn"]["n_eval_rollout_threads"] = vec_env.task.num_envs_val    # override the config

        self.envs = vec_env
        self.eval_envs = vec_env
        # parameters
        self.env_name = vec_env.task.cfg["env"]["env_name"]
        self.algorithm_name = config["learn"]["algorithm_name"]
        self.experiment_name = config["learn"]["experiment_name"]
        self.use_centralized_V = config["learn"]["use_centralized_V"]
        self.use_obs_instead_of_state = config["learn"]["use_obs_instead_of_state"]
        self.num_env_steps = config["learn"]["num_env_steps"]
        self.episode_length = config["learn"]["episode_length"]
        self.n_rollout_threads = config["learn"]["n_rollout_threads"]
        self.n_eval_rollout_threads = config["learn"]["n_eval_rollout_threads"]
        self.use_linear_lr_decay = config["learn"]["use_linear_lr_decay"]
        self.hidden_size = config["learn"]["hidden_size"]
        self.use_render = config["learn"]["use_render"]
        self.recurrent_N = config["learn"]["recurrent_N"]
        self.use_single_network = config["learn"]["use_single_network"]
        # interval
        self.save_interval = config["learn"]["save_interval"]
        self.use_eval = config["learn"]["use_eval"]
        self.eval_interval = config["learn"]["eval_interval"]
        self.eval_episodes = config["learn"]["eval_episodes"]
        self.log_interval = config["learn"]["log_interval"]
        ...
        config["learn"]["task_meta"] = vec_env.task.task_meta

        self.run_dir = config["learn"]["run_dir"]
        ...
        for agent_id in range(self.num_agents):
            ...
            # buffer
            share_observation_space = self.envs.share_observation_space[agent_id] if self.use_centralized_V else self.envs.observation_space[agent_id]
            bu = SeparatedReplayBuffer(config["learn"],
                                       self.envs.observation_space[agent_id],
                                       share_observation_space,
                                       self.envs.action_space[agent_id],
                                       device=self.device)
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            if episode % self.log_interval == 0:
                ...
                print("\nAlgo {} Exp {} updates {}/{} episodes, total num timesteps {}/{}, FPS {}, success rate {}.\n"
                      .format(self.algorithm_name,
                              ...
                              self.envs.task.extras["success_rate"]))
    ...
    def warmup(self):
        ...
        for agent_id in range(self.num_agents):
            # print(self.buffer[agent_id].share_obs[0].shape, share_obs[:, agent_id].shape)
    ...
    def restore(self):
        for agent_id in range(self.num_agents):
            if self.use_single_network:
                policy_model_state_dict = torch.load(str(self.model_dir) + '/model_agent' + str(agent_id) + '.pt', map_location=self.device)
    ...
    def eval(self, total_num_steps):
        ...
        total_done = torch.zeros((self.n_eval_rollout_threads, self.num_agents), device=self.device)
        total_success = torch.zeros((self.n_eval_rollout_threads), device=self.device)
        ...
        while True:
            ...
            total_done += eval_dones
            total_success += self.envs.task.extras["successes"]
            ...
            if eval_episode >= self.eval_episodes:
                ...
                eval_env_infos = {'eval_average_episode_rewards': torch.mean(eval_episode_rewards),
                                  ...
                                  'eval_success_rate': total_success/(total_done.sum(dim=1)+1e-8)}
